      ...
      {clients.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-lg border border-border bg-card shadow-card">
          <table className="w-full min-w-[960px] text-left text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/60 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
              ...
              {clients.map((client) => {
                ...
                return (
                  <tr key={client.id} className="group">
                    ...
                    <td className="px-4 py-3 text-sm text-foreground">{client.accountManager?.label ?? <span className="text-xs text-muted-foreground">Unassigned</span>}</td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      {client.integrations.length === 0 ? (
                        ...
                        <div className="flex -space-x-0.5" title={client.integrations.map((i) => `${PROVIDER_LABEL[i.provider]}: ${i.status}`).join(', ')}>
                          ...
                          {client.integrations.length > 6 && <span className="pl-1.5 text-xs text-muted-foreground">+{client.integrations.length - 6}</span>}
                        </div>
                      )}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="whitespace-nowrap px-4 py-3 text-xs tabular-nums text-muted-foreground">{formatRelative(client.lastActivityAt)}</td>
